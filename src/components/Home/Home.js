...
class Home extends React.Component {
    ...
    componentDidMount() {
        ...
        this.notificationListener = firebase.notifications().onNotification(async (notification) => {
            ...
            if (notification.title === "New Message" && NavigationService.getRouteName() === 'ChatScreen' && NavigationService.getParams().sender === body[0]) {
                ...
                <TouchableOpacity onPress={() => { this.onClickSubmission(item.submissionId) }}>
                    <View style={styles.cardView}>
                        ...
                        <View style={{ flex: 1, padding: 5 }}>
                            <Text style={{ flex: 1, textAlign: 'left', fontSize: 14, fontWeight: 'bold' }}>{item.actionUser} did a submission</Text>
                            <Text style={{ textAlign: 'left', fontSize: 12, marginTop: 3 }}><Text style={styles.boldText}>Problem</Text>: {item.problemCode}    <Text style={styles.boldText}>Contest</Text>: {item.contestCode}</Text>
                            <Text style={{ textAlign: 'left', fontSize: 12, marginTop: 3 }}><Text style={styles.boldText}>Verdict</Text>: {item.submissionResult}   <Text style={styles.boldText}>Time</Text>: {this.msToTime(item.feedDate)} GMT</Text>
                        </View>
                    </View>
                </TouchableOpacity>
            )
                ...
                <TouchableOpacity onPress={() => { this.onClickContest(item) }}>
                    <View style={styles.cardView}>
                        ...
                        <View style={{ flex: 1, padding: 5 }}>
                            ...
                            <Text style={{ textAlign: 'left', fontSize: 12, marginTop: 3 }}><Text style={styles.boldText}>Start Time</Text>: {item.contestStartDate} IST </Text>
                            <Text style={{ textAlign: 'left', fontSize: 12, marginTop: 3 }}><Text style={styles.boldText}>End Time</Text>: {item.contestEndDate} IST</Text>
                        </View>
                    </View>
                </TouchableOpacity>
            )
