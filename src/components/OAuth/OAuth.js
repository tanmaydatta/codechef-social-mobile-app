...
const url = 'https://api.codechef.com/oauth/authorize?response_type=code&client_id=7cecd56796986a7d2d5d9f8a78b050f1&redirect_uri=codechefsocial://launch&state=xyz'
        ...
        Network.getAccessToken(code).then(async (data) => {
            if (data.scope.split(' ').length == 5) {
                ...
                Network.getUserProfile('me').then(async userData => {
                    ...
                    Network.registerUser(userData.username, data.access_token, data.refresh_token).then(finalData => {
                        firebase.messaging().getToken()
                            .then(fcmToken => {
                                if (fcmToken) {
                                    Network.registerFcm(fcmToken, data.access_token).then(registeredData => {
