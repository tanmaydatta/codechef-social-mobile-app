...
import { Platform, TouchableOpacity, Image, RefreshControl, TextInput, ScrollView, View, FlatList, Text, StyleSheet } from 'react-native';
...
class Todo extends Component {
	constructor(props) {
		super(props);
		this.state = { data: null, wholeData: null, animating: true, reloadTable: true }
	...
	renderRow = ({ item }) => (
		<TodoRow
			item={item}
			onDelete={this.reloadFromDelete.bind(this)}
		/>
	);
	...
	onSearch(text) {
		if (text) {
			...
			data = data.filter(item => {
				return item.problemName.toLowerCase().indexOf(text.toString().toLowerCase()) > -1;
			});
			console.log(data);
			this.setState({
				...this.state,
			...
			return (
				<View style={styles.container}>
					<View style={styles.toolbar}>
						...
						<TouchableOpacity style={{ height: 20, width: 20, marginRight: 10 }} onPress={() => { this._onPressAdd() }}>
							<Image style={{ height: 20, width: 20, marginRight: 10 }} source={require("../../images/add_header.png")} />
						</TouchableOpacity>
					</View>
