...
class TodoRow extends Component {
  constructor(props) {
    ...
    this.state = {
      item: props.item,
      attemptedIcon:
        props.item.status == "attempted" ? "close" : "error-outline",
      attemptedIconColor:
        props.item.status == "attempted" ? "#e74c3c" : "#2f3640"
    };
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: "OK",
          onPress: () =>
            Network.deleteTodo(this.props.item.problemCode).then(data => {
              console.log(data);
              realm.write(() => {
                realm.delete(
                  realm.objectForPrimaryKey(
                    "TodoProblems",
                    this.props.item.problemCode
                  )
                );
              });
              {
  ...
  render() {
    ...
    return (
      <View key={this.props.item.problemCode} style={styles.cardView}>
        <Image
          style={{ height: 25, width: 25, alignSelf: "center", marginLeft: 4 }}
          source={icon}
        />
        <View style={styles.rowContent}>
          <TouchableOpacity
            onPress={() => {
              this._onPressProblem(this.props.item);
            }}
          >
            <Text style={styles.problemName}>
              {this.props.item.problemName}
            </Text>
          </TouchableOpacity>
        ...
        <TouchableOpacity
          style={{ width: 25, height: 25, alignSelf: "center", marginRight: 4 }}
