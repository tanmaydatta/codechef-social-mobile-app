...
class Search extends Component {
  ...
  onSearch(text) {
    if (text) {
      ...
      if (type == "addSetUser" || type == "returnUsername") {
        Network.getUsers("username", 10, 0, text)
          .then(data => {
            console.log(data);
  ...
  addUser(memberHandle) {
    ...
    Network.addFriend(setName, memberHandle)
      .then(data => {
        console.log(data);
