...
class NotificationScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    let oldPosts = realm
      .objects("NotificationFeed")
      .filtered(millisecondsQuery);
    realm.write(() => {
  ...
  async refresh(lastUpdatedAt) {
    ...
    Network.getNotifications(lastUpdatedAt, false).then(async data => {
      ...
      if (data != null) {
        ...
        this.setState({
          data: this.convertToArray(realm.objects("NotificationFeed"))
        });
      }
    });
  }
        ...
        >
          <View style={styles.cardView}>
            ...
            <View style={{ flex: 1, padding: 5 }}>
              ...
              >
                {item.actionUser} did a {item.submissionResult} submission of{" "}
                {item.problemCode}
              </Text>
