...
class Network {
        ...
        return fetch(url, {
            ...
            .then(async responseJson => {
                ...
                if (responseJson.status == "Forbidden") {
                    ...
                    return responseJson
                } else if (!responseJson.result.data || responseJson.status == "error") {
                    if (repeated) {
                        this.logout()
                    } else {
                        const data = await this.refreshToken()
                        const response = await this.apiCall(url, method, headers, body, flag, true)
                        return response
                    }
                } else {
                    if (flag) {
            ...
            .then(async responseJson => {
                ...
                if (responseJson.status != 200) {
                    if (repeated) {
                        ...
                        const data = await this.refreshToken()
                        const response = await this.apiCustomCall(url, method, headers, body, true)
                        return response
                    }
                }
                else {
    ...
    static async refreshToken() {
        ...
        return fetch(alibabaUrl + '/codechef-social-server/api/users/getNewToken', {
            ...
            .then(async (responseJson) => {
                ...
                console.log(responseJson)
                if (responseJson.status == 200) {
                    await AsyncStorage.setItem('accessToken', responseJson.result.accessToken)
                    return responseJson
                } else {
                    this.logout()
                }
    ...
    static async deleteFriend(setName, memberName) {
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCall(
            codechefUrl + "sets/members/delete?setName=" + setName + "&memberHandle=" + memberName,
            "DELETE",
            {
                Accept: "application/json",
                Authorization: "Bearer " + val
    ...
    static async getContestProblems(contest) {
        ...
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCall(
            codechefUrl + "contests/" + contest + "?fields=problemsList,announcements",
            "GET",
            {
                Accept: "application/json",
                Authorization: "Bearer " + val
    ...
    static async getUsers(fields, limit, offset, search) {
        ...
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCall(
            'https://api.codechef.com/users?fields=' + fields + '&limit=' + limit + '&offset=' + offset + '&search=' + search,
            "GET",
            {
                Accept: "application/json",
                Authorization: "Bearer " + val
    ...
    static async getFeed(lastUpdatedAt) {
        ...
        console.log(val)
        return this.apiCustomCall(
            alibabaUrl + '/codechef-social-server/api/users/feed/get?lastUpdatedAt=' + lastUpdatedAt,
            "GET",
            {
                'Content-Type': 'application/json',
                'token': val,
    ...
    static async getNotifications(lastUpdatedAt) {
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCustomCall(
            alibabaUrl + '/codechef-social-server/api/users/notifications/get?lastUpdatedAt=' + lastUpdatedAt,
            "GET",
            {
                'token': val
            },
    ...
    static async getMessages(user, lastUpdatedAt) {
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCustomCall(
            alibabaUrl + '/codechef-social-server/api/users/messages/' + user + '/get?lastUpdatedAt=' + lastUpdatedAt,
            "GET",
            {
                'token': val
            },
    ...
    static async getRatings(contestType, offset) {
        const val = await AsyncStorage.getItem('accessToken');
        return this.apiCall(
            'https://api.codechef.com/ratings/' + contestType + '?limit=25&offset=' + offset,
            "GET",
            {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
